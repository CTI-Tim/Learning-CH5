...
namespace ContractExample
{
    static class MessageBroker  // Static because everyone is going to use this everywhere
    {
         ...
         *
         *
         *  we need to create a delegate signature for our messaging we we are going to pass generic objects here.
         *  Generic Objects are chosen so this is not tightly coupled and becomes useable for nearly anything.
         *
        ...
        {
            if (_messages.ContainsKey(key))
                _messages[key].DynamicInvoke(message);
            else
            {
